package com.juyoufuli.cloud.service.impl;
...
public class RedisServiceImpl<T> implements RedisService<T> {
    ...
    public long incr(String key, long incr) {
        if (incr < 0) {
            throw new RuntimeException("递增incr必须大于0");
        }
     ...
     */
    @Override
    public long decr(String key, long incr) {
        if (incr > 0) {
            throw new RuntimeException("递减incr必须小于0");
        }
        return redisTemplate.opsForValue().increment(key, incr);
    }
     ...
     */
    @Override
    public List<Object> hmget(String key, List<Object> item){
