...
import com.juyoufuli.common.io.IOUtils;
import com.juyoufuli.common.reflect.ReflectUtils;
...
//							}
	 ...
	 * @param obj
	 */
	public static void toObjectIgnoreNull(Object obj) {
		...
		for (Field field : fields) {
			try {
				...
				if (t == String.class) {
					Object name = ReflectUtils.invokeGetter(obj, field.getName());// 调用该字段的get方法
					if (name == null) {
						ReflectUtils.invokeSetter(obj, field.getName(), "");// 调用该字段的get方法// 执行相应赋值方法
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
